...
const findHappiness = (arr) => {
  let newArr = [];
  arr.forEach(str => str.includes(':)')?newArr.push(str):0);
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(str => str.slice(1,4)+str.slice(6,9)+str.slice(10));
};
...
const onlyOddChars = (str) => {
  let newStr = '';
  str.split('').forEach((char,idx) => idx%2 === 1 ? newStr += char : 0);
  return newStr;
};
...
const allHappy = (arr) => {
  let newBool = true;
  arr.forEach(str => newBool === true ? newBool = str.includes(':)') : 0);
  return newBool;
};
...
const findAnything = (arr, target) => {
  let newArr = [];
  arr.forEach(str => str.includes(target)?newArr.push(str):0);
  return newArr;
};
...
const findEvery = (arr, target) => {
  let newBool = true;
  arr.forEach(str => newBool === true ? newBool = str.includes(target) : 0);
  return newBool;
};
...
const unenrollBrook = (arr) => {
  ...
  arr.forEach(subArr => {
    let newSubArr = [];
    subArr.forEach(str => str.toUpperCase().includes('BROOK') ? 0 : newSubArr.push(str));
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach(day => {
    let newSubArr = [];
    arr.forEach(str => str.toUpperCase().includes(day.toUpperCase()) ? newSubArr.push(str) : 0);
    newArr.push(newSubArr);
  });
  return newArr;
};
