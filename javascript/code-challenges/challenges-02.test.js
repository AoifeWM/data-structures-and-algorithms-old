...
const mapTwoToThe = (arr) => {
  return arr.map(num => num = Math.pow(2, num));
};
...
const charCode = (arr) => {
  return arr.map(str => str = str.charCodeAt(0));
};
...
const extractAbilities = (arr) => {
  let newArr = [];
  arr.map(obj => newArr.push(obj.ability.name));
  return newArr;
};
...
const extractStats = (arr) => {
  let newArr = [];
  arr.map(obj => newArr.push({name:obj.stat.name, total:(obj.effort+obj.baseStat)}));
  return newArr;
};
