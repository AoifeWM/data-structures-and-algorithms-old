...
const screenForNames = (arr) => {
  let newArr = [];
  arr.forEach(str => str.search(/^(Dr\.|Mr\.|Mrs\.|Ms\.)\s[A-Za-z][A-Za-z\s]+$/) === -1 ? 0 : newArr.push(str));
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  let newStr = '';
  arr.forEach(obj => obj.mass > 77 ? (newStr === '' ? newStr = obj.name : newStr += ` - ${obj.name}`) : 0);
  return newStr;
};
...
const sortBy = (property, arr) => {
  return arr.sort((a,b) => typeof a[property] === 'number' ? a[property] - b[property] : a[property].localeCompare(b[property]));
};
