...
function transformToLis(obj){
  let newArr = [];
  Object.keys(obj).forEach((key, idx) => newArr.push(`<li>${key}: ${Object.values(obj)[idx]}</li>`));
  return newArr;
}
...
const count = (target, input) => {
  let counter = 0;
  input.forEach(arr => arr.forEach(int => int === target ? counter += 1 : 0));
  return counter;
};
...
const totalSum = (input) => {
  return input.map(arr => arr.reduce((a,b) => a+b,0)).reduce((a,b) => a+b,0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(arr => {
    let tempArr = [];
    arr.forEach(int => Number.isInteger(int) ? (int % 5 === 0 ? tempArr.push(Math.pow(2, int)) : 0): 0);
...
let findMaleAndFemale = (data) => {
  let returnStr = '';
  data.forEach(obj => obj.gender === 'male' || obj.gender === 'female' ? (returnStr === '' ? returnStr = obj.name : returnStr += ` and ${obj.name}`) : 0);
  return returnStr;
};
...
let findShortest = (data) => {
  return data.reduce((a,b) => a.height<b.height?b:a).name;
};
