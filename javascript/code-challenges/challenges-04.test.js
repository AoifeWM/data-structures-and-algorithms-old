...
const updateAnimal = (arr, callback) => {
  return arr.map(item => {return item = callback(item);});
};
...
const sortMeetingsByDay = (arr) => {
  return arr.sort((a,b) => {
    return weekdayToNum(a.dayOfWeek)-weekdayToNum(b.dayOfWeek);
  });
};

const weekdayToNum = (str) => {
  let days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
  return days.indexOf(str.toLowerCase());
};
...
const sortSchedule = (arr) => {
  return arr.sort((a,b) => {
    if(weekdayToNum(a.dayOfWeek)-weekdayToNum(b.dayOfWeek) !== 0){
      return weekdayToNum(a.dayOfWeek)-weekdayToNum(b.dayOfWeek);
