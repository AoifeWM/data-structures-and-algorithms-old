...
const sortByChildren = (charArray) => {
  return charArray.sort((objA, objB) => {
    if (objA.children.length === objB.children.length){
      return objA.house.localeCompare(objB.house);
    }else{
...
const isNum = (input) => {
  return -1 !== input.toString().search(/[0-9]/);
};
...
const containsWorld = (input) => {
  return -1 !== input.toString().search(/world/);
};
...
const matchMonth = (input) => {
  if(typeof input !== 'string'){
    return false;
  }else if(-1 !== input.search(/^oct$/)){
    return true;
  }else if(-1 !== input.search(/^Oct$/)){
    return true;
  }else if(-1 !== input.search(/^October$/)){
    return true;
  }else if(-1 !== input.search(/^october$/)){
    return true;
  }else{
