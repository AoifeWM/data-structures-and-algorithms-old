...
const howMuchPencil = (str) => {
  let result = [];
  for(let i = str.length; i > -1; i--){result.push(str.split('').splice(-i, i).join(''));}
  return result;
};
...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(str => result.push(str.slice(str.indexOf(' ',3)+1)));
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(str => result.push(str.split(' ').splice(2).join(' ')));
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  newArr.reverse().forEach(int => arr.splice(int,1));
};
...
const totalSumCSV = (str) => {
  return str.split(',').map(str => parseInt(str)).reduce((a,b)=>a+b);
};
...
const removeVowels = (str) => {
  ...
  let newArr = [];
  splitString.forEach((str, idx) => {
    if(vowels.includes(str)){
      newArr.push(idx);
    }
  });
  newArr.reverse().forEach(int => splitString.splice(int,1));
...
const extractVowels = (str) => {
  ...
  idxArr.reverse().forEach(int => vowellArr.push(splitString.splice(int,1).join()));
  return [splitString.join(''),vowellArr.sort((a, b) => a.localeCompare(b)).join('')];
};
